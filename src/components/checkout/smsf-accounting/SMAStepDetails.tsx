...
export const SMAStepDetails: React.FC<Props> = ({ onNext, onBack }) => {
  const { customer, updateCustomer } = useCheckout();
  const [fundName, setFundName] = useState((customer.smaFundName as string) || "");
  const [abn, setAbn] = useState((customer.smaABN as string) || "");
  const [trusteeName, setTrusteeName] = useState((customer.smaTrusteeName as string) || "");
  const [fullName, setFullName] = useState((customer.smaFullName as string) || "");
  const [email, setEmail] = useState((customer.smaEmail as string) || "");
  const [phone, setPhone] = useState((customer.smaPhone as string) || "");
  const [errors, setErrors] = useState<Record<string, string>>({});

  const validate = (): boolean => {
    const e: Record<string, string> = {};
    if (!fundName.trim()) e.fundName = "Fund name is required";
  ...
  const isFormValid = !!(fundName.trim() && trusteeName.trim() && fullName.trim() && abn.trim() && email.trim() && phone.trim());
  const handleContinue = () => {
    if (!validate()) return;
    updateCustomer({ smaFundName: fundName, smaABN: abn, smaTrusteeName: trusteeName, smaFullName: fullName, smaEmail: email, smaPhone: phone });
    onNext();
  };

  return (
    <div className="space-y-6">
      <div><h2 className="text-2xl font-bold text-foreground">SMSF Details</h2><p className="text-muted-foreground mt-1">Tell us about your self-managed super fund</p></div>
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div><label className="block text-sm font-medium text-foreground mb-1.5">SMSF Name <span className="text-destructive">*</span></label><Input value={fundName} onChange={(e) => setFundName(e.target.value)} placeholder="Smith Family Super Fund" className={errors.fundName ? "border-destructive" : ""} />{errors.fundName && <p className="text-destructive text-sm mt-1">{errors.fundName}</p>}</div>
      ...
      <div><label className="block text-sm font-medium text-foreground mb-1.5">Trustee Name <span className="text-destructive">*</span></label><Input value={trusteeName} onChange={(e) => setTrusteeName(e.target.value)} placeholder="Smith Super Pty Ltd" className={errors.trusteeName ? "border-destructive" : ""} />{errors.trusteeName && <p className="text-destructive text-sm mt-1">{errors.trusteeName}</p>}</div>
