...
const steps = [
  ...
  { number: 3, label: "Organization Details", sub: "Your information" },
...
export const NFPStepper: React.FC<Props> = ({ currentStep, onStepClick }) => {
          ...
          return (
            <React.Fragment key={step.number}>
              ...
              {index < steps.length - 1 && (
                <div className="flex-1 flex items-center px-1 md:px-3 -mt-7 md:-mt-7">
                  <div className={`h-0.5 w-full transition-colors ${step.number < currentStep ? "bg-[hsl(var(--success))]" : "bg-border"}`} />
                </div>
              )}
            </React.Fragment>
          );
