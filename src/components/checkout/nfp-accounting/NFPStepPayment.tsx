...
export const NFPStepPayment: React.FC<Props> = ({ onBack }) => {
  ...
  const revenueTier = (customer.nfpRevenue as string) || "under75k";
  const billing = (customer.nfpBilling as "monthly" | "annual") || "annual";
  const plan = (customer.nfpPackageLevel as "essential" | "premium") || "essential";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await submitCheckout({
        ...
        customer: {
          orgName: customer.nfpOrgName, state: customer.nfpState, incNumber: customer.nfpIncNumber,
          abn: customer.nfpABN, fullName: customer.nfpFullName, email: customer.nfpEmail,
          phone: customer.nfpPhone, taxAgentMethod: customer.nfpTaxAgentMethod, termsAccepted,
        },
        selections: {
      ...
      <div className="space-y-6">
        ...
        <div className="border border-border rounded-xl p-5">
          <div className="flex items-start gap-3">
            <Checkbox id="nfp-terms" checked={termsAccepted} onCheckedChange={(checked) => setTermsAccepted(!!checked)} className="mt-0.5" />
            <label htmlFor="nfp-terms" className="text-sm text-placeholder cursor-pointer">
        ...
        <div className="border-2 border-[hsl(var(--cta)/0.2)] rounded-xl p-5">
          ...
          <div className="space-y-4">
            {[
              ...
              { step: 2, title: "Account Manager Contact (Within 24 Hours)", desc: "Your dedicated NFP Accounting Specialist will reach out to welcome you" },
              ...
              { step: 5, title: "Ongoing Support & Reporting", desc: "Financial reports, ACNC compliance, grant acquittals, and unlimited email support" },
            ].map((item) => (
