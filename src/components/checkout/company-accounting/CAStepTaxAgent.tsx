...
export const CAStepTaxAgent: React.FC<Props> = ({ onNext, onBack }) => {
  const { customer, updateCustomer } = useCheckout();
  const [method, setMethod] = useState((customer.caTaxAgentMethod as string) || "mygov");

  const handleContinue = () => {
    updateCustomer({ caTaxAgentMethod: method });
    onNext();
  };
              ...
              <div className="flex items-start gap-3">
                <div
                  className={cn(
                    ...
                    method === opt.id ? "border-[hsl(var(--success))]" : "border-muted-foreground/40"
                  )}
                >
