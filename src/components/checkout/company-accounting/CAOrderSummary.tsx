...
export const CAOrderSummary: React.FC = () => {
  ...
  const showStrike = isAnnual; // In annual mode, show pre-discount vs post-discount per reference
    ...
    <div className="bg-card rounded-2xl border border-border overflow-hidden sticky top-6">
      ...
      <div className="p-5 space-y-4">
        ...
        <div className="space-y-3 text-sm">
          <div className="flex justify-between">
            <div>
              ...
              <TooltipProvider><Tooltip><TooltipTrigger asChild><p className="text-xs text-placeholder flex items-center gap-1 cursor-help">Fixed fee <Info size={12} /></p></TooltipTrigger><TooltipContent className="max-w-[240px]"><p className="text-xs">Annual financial statements and tax return compliance fee.</p></TooltipContent></Tooltip></TooltipProvider>
            </div>
            {isAnnual ? (
              <div className="text-right">
                <span className="text-xs text-muted-foreground/60 line-through mr-1">${result.compliance.toLocaleString()}</span>
                <span className="font-medium text-foreground">${Math.round(result.compliance * (1 - cfg.annualDiscount)).toLocaleString()}</span>
              </div>
            ) : (
              <span className="font-medium text-foreground">${Math.round(result.compliance / 12).toLocaleString()}/mo</span>
            )}
          </div>

          <div className="flex justify-between">
            <div>
              <p className="font-medium text-foreground">Ongoing Accounting & BAS ({months} months)</p>
              <p className="text-xs text-placeholder">${result.monthlyFee}/mo{isAnnual ? ` × ${months}` : ""}</p>
            </div>
            {isAnnual ? (
              <div className="text-right">
                <span className="text-xs text-muted-foreground/60 line-through mr-1">${result.operations.toLocaleString()}</span>
                <span className="font-medium text-foreground">${Math.round(result.operations * (1 - cfg.annualDiscount)).toLocaleString()}</span>
              </div>
            ) : (
              <span className="font-medium text-foreground">${result.monthlyFee?.toLocaleString()}/mo</span>
            )}
          </div>
          ...
          {payrollFee > 0 && (
            <div className="flex justify-between">
              <div><p className="text-foreground">Payroll Services</p><p className="text-xs text-placeholder">{employeeCount} employee × {months} months</p></div>
              <span className="font-medium text-foreground">${payrollFee.toLocaleString()}{billing === "monthly" ? "/mo" : ""}</span>
            </div>
          )}

          {catchUpFee > 0 && (
            <div className="flex justify-between">
              <div><p className="text-foreground">Catch-Up Pack</p><p className="text-xs text-placeholder">Financial review & clean-up (one-time)</p></div>
              <span className="font-medium text-foreground">${catchUpFee.toLocaleString()}</span>
            </div>
          )}

          {officeFee > 0 && (
            <div className="flex justify-between">
              <div><p className="text-foreground">Registered Office</p><p className="text-xs text-placeholder">{months} months</p></div>
              <span className="font-medium text-foreground">${officeFee.toLocaleString()}</span>
            </div>
          )}

          {taxPlanningFee > 0 && (
            <div className="flex justify-between">
              <div><p className="text-foreground">Tax Planning Session</p></div>
              <span className="font-medium text-foreground">${taxPlanningFee.toLocaleString()}</span>
            </div>
          )}
        </div>

        {isAnnual && result.discount > 0 && (
        ...
        <div className="bg-[hsl(var(--cta)/0.05)] border border-[hsl(var(--cta)/0.2)] rounded-xl p-4">
          ...
          {isAnnual && (
            <p className="text-sm text-muted-foreground/60 line-through mt-0.5">${(result.compliance + result.operations + (result.transition || 0) + (result.premiumExtra || 0) + addonsTotal).toLocaleString()}</p>
          )}
          ...
          <p className="text-xs text-placeholder mt-1">
            {billing === "monthly" ? "Billed monthly · cancel anytime · no annual discount" : `One-time payment for ${months} months · ${Math.round(cfg.annualDiscount * 100)}% discount applied`}
          </p>
        </div>
