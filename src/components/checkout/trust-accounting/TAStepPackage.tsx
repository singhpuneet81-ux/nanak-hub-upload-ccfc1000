...
export const TAStepPackage: React.FC<Props> = ({ onNext }) => {
  ...
  const startDate = (customer.taStartDate as string) || "jul";
  ...
  useEffect(() => {
    ...
    if (!customer.taStartDate || !customer.taRevenue || !customer.taBilling || !customer.taPackageLevel) {
      updateCustomer({
        taStartDate: initStart,
        taRevenue: initRevenue,
        taBilling: initBilling,
        taPackageLevel: initPlan,
      });
    }
  }, []);
    ...
    <div className="space-y-8">
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
          {cfg.startDates.map((d) => (
            <button
              key={d.id}
              onClick={() => updateCustomer({ taStartDate: d.id })}
              className={cn(
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <Select value={revenueTier} onValueChange={(v) => updateCustomer({ taRevenue: v })}>
        ...
        <div>
          ...
          <div className="inline-flex items-center gap-1 bg-primary/10 border-2 border-primary/20 rounded-full p-1.5">
            <button onClick={() => updateCustomer({ taBilling: "monthly" })} className={cn("px-5 py-2 rounded-full text-sm font-semibold transition-all", billing === "monthly" ? "bg-[hsl(var(--cta))] text-white shadow-lg disabled:opacity-50" : "text-placeholder hover:text-foreground")}>Monthly</button>
            <button onClick={() => updateCustomer({ taBilling: "annual" })} className={cn("px-5 py-2 rounded-full text-sm font-semibold transition-all flex items-center gap-1.5", billing === "annual" ? "bg-[hsl(var(--cta))] text-white shadow-lg disabled:opacity-50" : "text-placeholder hover:text-foreground")}>
              Annual
      ...
      <div>
        ...
        <div className="bg-primary/5 border border-primary/20 rounded-xl p-4 mb-5">
          <p className="text-2xl font-bold text-foreground">
            ...
            <span className="text-sm font-normal text-placeholder">{billing === "monthly" ? "/mo" : `/${months}mo`}</span>
          </p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div onClick={() => updateCustomer({ taPackageLevel: "essential" })} className={cn("relative rounded-2xl border-2 p-6 cursor-pointer transition-all", packageLevel === "essential" ? "border-primary bg-primary/5" : "border-border hover:border-primary/40")}>
            ...
            <div className="space-y-2">
              {essentialPlan.features.map((f, i) => (
                <div key={i} className="flex items-center gap-2"><Check className="w-4 h-4 text-[hsl(var(--success))]" /><span className="text-sm text-foreground">{f}</span></div>
              ))}
              {essentialPlan.extraFeatures.length > 0 && <ExpandableServices accentColor="primary" extraServices={essentialPlan.extraFeatures} />}
            </div>
          </div>

          <div onClick={() => updateCustomer({ taPackageLevel: "premium" })} className={cn("relative rounded-2xl border-2 p-6 cursor-pointer transition-all", packageLevel === "premium" ? "border-[hsl(var(--cta))] bg-[hsl(var(--cta)/0.03)]" : "border-border hover:border-primary/40")}>
            {premiumPlan.badge && (
              <div className="absolute -top-3 left-4">
                <span className="bg-[hsl(var(--cta))] text-white text-xs font-bold px-3 py-1 rounded-full flex items-center gap-1 disabled:opacity-50"><Zap size={12} /> {premiumPlan.badge}</span>
              </div>
            )}
      ...
      <TooltipProvider>
        <Tooltip>
          <TooltipTrigger asChild>
            <p className="text-xs text-placeholder flex items-center gap-1 cursor-help"><Info size={12} /> Annual compliance fees are fixed deliverables and not subject to proration.</p>
          </TooltipTrigger>
          <TooltipContent className="max-w-[280px]"><p className="text-xs">Annual financial statements and tax return are fixed compliance deliverables and not subject to proration.</p></TooltipContent>
        </Tooltip>
      </TooltipProvider>

      <div className="checkout-nav flex justify-center pt-2">
        <button onClick={onNext} className="flex items-center gap-2 px-8 py-3 bg-[hsl(var(--cta))] text-white rounded-lg font-medium hover:opacity-90 transition-opacity disabled:opacity-50">
