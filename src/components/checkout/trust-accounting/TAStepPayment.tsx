...
export const TAStepPayment: React.FC<Props> = ({ onBack }) => {
  ...
  const plan = (customer.taPackageLevel as "essential" | "premium") || "essential";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await submitCheckout({
        ...
        customer: {
          trustName: customer.taTrustName, abn: customer.taABN, trusteeName: customer.taTrusteeName,
          fullName: customer.taFullName, email: customer.taEmail, phone: customer.taPhone,
          taxAgentMethod: customer.taTaxAgentMethod, termsAccepted,
        },
        selections: {
      ...
      <div className="space-y-6">
        ...
        <div className="border border-border rounded-xl p-5">
          <div className="flex items-start gap-3">
            <Checkbox id="ta-terms" checked={termsAccepted} onCheckedChange={(checked) => setTermsAccepted(!!checked)} className="mt-0.5" />
            <label htmlFor="ta-terms" className="text-sm text-placeholder cursor-pointer">
