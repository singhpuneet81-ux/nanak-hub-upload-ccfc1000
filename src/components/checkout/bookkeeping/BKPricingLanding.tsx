...
import { Check, ArrowRight, Zap, Target, CheckCircle2 } from "lucide-react";
import { cn } from "@/lib/utils";
import { useBookkeepingPricing } from "@/hooks/useBookkeepingPricing";
import { BK_TIERS } from "./bkPricing";

export const BKPricingLanding: React.FC = () => {
  ...
  const handleGetStarted = (tierIdx: number) => {
    ...
    window.open(`${baseUrl}/bookkeeping?checkout=1&tier=${tierIdx}`, "_blank", "noopener,noreferrer");
  };
      ...
      <div className="py-4 px-4">
        <div className="max-w-5xl mx-auto">
                ...
                >
                  ...
                  <div className={cn("p-6", isPopular && "pt-4")}>
                    <p className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wider mb-1">
                      {staticTier?.txnLabel || `UP TO ${t.txn} TXNS/MO`}
                    </p>
          ...
          <p className="text-center text-xs text-muted-foreground mt-6">
            All prices exclude GST · {isAnnual ? `${discountPct}% annual discount applied` : "Switch to annual to save"} · BAS preparation always included
          </p>
        </div>
      </div>
    </div>
