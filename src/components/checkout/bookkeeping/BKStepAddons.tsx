...
export const BKStepAddons: React.FC<Props> = ({ onNext, onBack }) => {
  const { customer, updateCustomer } = useCheckout();

  const catchUpNeeded = (customer.bkPremiumCatchUp as string) || "up_to_date";
    ...
    <div className="space-y-6">
      ...
      <div className="border-2 border-border rounded-xl p-5">
        ...
        <p className="text-2xl font-bold text-[hsl(var(--cta))] mb-4">
          ${CATCH_UP_FEE} <span className="text-sm font-normal text-muted-foreground">one-time fee</span>
        </p>

        <div className="border border-border rounded-lg overflow-hidden">
          <div
            onClick={() => updateCustomer({ bkPremiumCatchUp: "up_to_date" })}
            className={cn(
          ...
          >
            <Checkbox
              ...
              onCheckedChange={() => updateCustomer({ bkPremiumCatchUp: "up_to_date" })}
          ...
          <div
            onClick={() => updateCustomer({ bkPremiumCatchUp: "need_support" })}
            className={cn(
          ...
          >
            <Checkbox
              ...
              onCheckedChange={() => updateCustomer({ bkPremiumCatchUp: "need_support" })}
              onClick={(e) => e.stopPropagation()}
              className="mt-0.5"
            />
            <div>
      ...
      <div
        onClick={() => updateCustomer({ bkRegisteredOffice: !registeredOffice })}
        className={cn(
      ...
      >
        <div className="flex items-start gap-3">
          <Checkbox
            ...
            onCheckedChange={(checked) => updateCustomer({ bkRegisteredOffice: !!checked })}
            onClick={(e) => e.stopPropagation()}
            className="mt-1"
          />
          <div>
            ...
            <p className="text-xl font-bold text-foreground mt-2">
              ${REGISTERED_OFFICE_FEE} <span className="text-sm font-normal text-muted-foreground">/year</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div
        onClick={() => updateCustomer({ bkTaxPlanning: !taxPlanning })}
        className={cn(
          "border-2 rounded-xl p-5 cursor-pointer transition-all",
          taxPlanning
            ? "border-[hsl(var(--cta))] bg-[hsl(var(--cta)/0.03)]"
      ...
      >
        <div className="flex items-start gap-3">
          <Checkbox
            checked={taxPlanning}
            onCheckedChange={(checked) => updateCustomer({ bkTaxPlanning: !!checked })}
            onClick={(e) => e.stopPropagation()}
            className="mt-1"
          />
          <div>
            ...
            <p className="text-xl font-bold text-foreground mt-2">
              ${TAX_PLANNING_FEE} <span className="text-sm font-normal text-muted-foreground">one-time</span>
            </p>
          </div>
        </div>
      </div>
