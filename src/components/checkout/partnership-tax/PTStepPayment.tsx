...
export const PTStepPayment: React.FC<Props> = ({ onBack }) => {
  ...
  const startDateId = (customer.ptStartDate as string) || "jul";
  const revenueTier = (customer.ptRevenue as string) || "under75k";
  const billing = (customer.ptBilling as "monthly" | "annual") || "annual";
  const plan = (customer.ptPackageLevel as "essential" | "premium") || "essential";
  const payrollEnabled = !!customer.ptPayroll;
  const employeeCount = (customer.ptEmployeeCount as number) || 1;
  const catchUp = (customer.ptCatchUp as string) || "up_to_date";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await submitCheckout({
        ...
        customer: {
          partnershipName: customer.ptPartnershipName, abn: customer.ptABN,
          partnerCount: customer.ptPartnerCount,
          fullName: customer.ptFullName, email: customer.ptEmail, phone: customer.ptPhone,
          taxAgentMethod: customer.ptTaxAgentMethod, termsAccepted,
        },
        selections: {
      ...
      <div className="space-y-6">
        ...
        <div className="border border-border rounded-xl p-5">
          <div className="flex items-start gap-3">
            <Checkbox id="pt-terms" checked={termsAccepted} onCheckedChange={(checked) => setTermsAccepted(!!checked)} className="mt-0.5" />
            <label htmlFor="pt-terms" className="text-sm text-placeholder cursor-pointer">
