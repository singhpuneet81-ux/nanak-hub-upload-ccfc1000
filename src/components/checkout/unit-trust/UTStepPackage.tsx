...
interface UTStepPackageProps {
  onNext: () => void;
  onBack: () => void;
}

export const UTStepPackage: React.FC<UTStepPackageProps> = ({
  onNext,
  onBack,
}) => {
  const { selections, updateSelections, customer, updateCustomer } = useCheckout();
  const { packages: pricingPackages, serviceMeta, isLoading } = usePricingPackages();
  const pricing = pricingPackages.unit_trust;

  // Extract API-driven packagePlans from serviceMeta
  const utMeta = serviceMeta?.unit_trust as { packagePlans?: ApiPackagePlans } | undefined;
  const packagePlans = utMeta?.packagePlans;
  ...
  >((selections.package as "registration_only" | "registration_plus_accounting") || "registration_plus_accounting");

  const [revenueBracket, setRevenueBracket] = useState(customer?.revenueBracket || "");
  const [billingFrequency, setBillingFrequency] = useState<"monthly" | "annual">(
    customer?.billingFrequency || "annual"
  );
  const [selectedPlan, setSelectedPlan] = useState<string>(customer?.accountingPlan || "");
  const [payrollEnabled, setPayrollEnabled] = useState(customer?.payrollEnabled || false);
  const [staffCount, setStaffCount] = useState(customer?.staffCount || 1);
  ...
  useEffect(() => {
    ...
    if (!selectedPlan || !apiPlansList.some(p => p.id === selectedPlan)) {
      setSelectedPlan(apiPlansList[0].id);
    }
    if (!revenueBracket || !revenueBrackets.some(b => b.id === revenueBracket)) {
      setRevenueBracket(revenueBrackets[0].id);
    }
  }, [apiPlansList, revenueBrackets]);

  useEffect(() => {
    updateSelections({ package: selectedPackage });
    updateCustomer({
      revenueBracket,
      billingFrequency,
  ...
  }, [selectedPackage, revenueBracket, billingFrequency, selectedPlan, payrollEnabled, staffCount]);
  ...
  const payrollPricePerStaff = 120;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        >
          ...
          <p className="text-sm text-muted-foreground mb-3">Just the essentials</p>
      ...
      {selectedPackage === "registration_plus_accounting" && (
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                <Select value={revenueBracket} onValueChange={setRevenueBracket}>
                  <SelectTrigger>
                    <SelectValue placeholder={isLoading ? "Loading..." : "Select revenue bracket"} />
                  </SelectTrigger>
                  <SelectContent>
                ...
                <div className="flex rounded-lg border border-border overflow-hidden">
                  <button
                    ...
                    className={cn(
                      ...
                      billingFrequency === "monthly" ? "bg-muted" : "bg-card hover:bg-muted/50"
                    )}
                  >
                    Monthly
                  </button>
                  <button
                    ...
                    className={cn(
                      "flex-1 py-2.5 text-sm font-medium transition-colors",
                      billingFrequency === "annual" ? "bg-primary text-primary-foreground" : "bg-card hover:bg-muted/50"
                    )}
                  >
                    Annual (Save)
                  </button>
          ...
          {/* Plan Selection Cards from API */}
          {apiPlansList.length > 0 && revenueBracket && (
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                {apiPlansList.map((plan, idx) => {
                  ...
                  const tierPrice = plan.tierPricing?.[revenueBracket];
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="flex items-center justify-between">
                  <Label htmlFor="payroll" className="font-semibold cursor-pointer">
                    Add Payroll Processing
                  </Label>
                  <div className="text-right">
                    <span className="font-bold text-foreground">${payrollPricePerStaff}</span>
                ...
                {payrollEnabled && (
                  <div className="mt-4 space-y-2">
                    ...
                    <div className="flex items-center gap-3">
                      <button type="button" onClick={() => setStaffCount(Math.max(1, staffCount - 1))} className="counter-btn-inline">
                        <Minus size={18} />
                      </button>
                      <span className="text-lg font-semibold w-8 text-center">{staffCount}</span>
                      <button type="button" onClick={() => setStaffCount(staffCount + 1)} className="counter-btn-inline">
                        <Plus size={18} />
                      </button>
                      <span className="text-sm text-muted-foreground ml-2">
                        = ${staffCount * payrollPricePerStaff}/yr
                      </span>
                    </div>
                  </div>
                )}
