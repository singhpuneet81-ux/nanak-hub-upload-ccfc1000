...
console.log("Selected plan:", selections.accountingPlan);
const handlePackageSelect = (packageId: string) => {
  if (packageId === "registration_only") {
    ...
    updateSelections({
      ...
      revenueBracket: selections.revenueBracket || "",
      billingFrequency: selections.billingFrequency ?? "monthly",
    accountingPlan: (selections.accountingPlan as AccountingPlanId) ?? "essential",
    });
  }
};

// const PLAN_ID_MAP: Record<string, AccountingPlanId> = {
//   essential: "essential",
...
// };
  ...
  const currentSavings =
    selectedBracket && selectedPlan && billingFrequency === "annual"
...
{showAccountingConfig && (
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
      <div className="space-y-2">
        ...
        <select
          ...
          onChange={(e) =>
            updateSelections({ revenueBracket: e.target.value })
          }
  ...
  onClick={() =>
    updateSelections({
      ...
      accountingPlan: selectedPlan ?? "essential",
    })
  }
  className={cn(
      ...
      <div className="space-y-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {accountingPlans.map((plan) => {
            const isSelected = selectedPlan === PLAN_ID_MAP[plan.id];
            ...
            return (
              ...
              onClick={() =>
updateSelections({
  accountingPlan: PLAN_ID_MAP[plan.id],
})
}
