/**
 * Trust Accounting — Compliance + Operations pricing model
 * Same structure as Company Accounting
 */

export type TARevenueKey =
  | "under75k"
  | "75to200k"
  | "200to500k"
  | "500to1m"
...
export interface TARevenueTierPricing {
  compliance: number;
  monthly: number;
}

export interface TAConfig {
  tiers: Record<TARevenueKey, TARevenueTierPricing>;
  annualDiscount: number;
  transitionFee: number;
}

export const TA_PRICING: TAConfig = {
  tiers: {
    under75k:    { compliance: 1000, monthly: 77 },
    "75to200k":  { compliance: 1200, monthly: 95 },
    "200to500k": { compliance: 1400, monthly: 115 },
    "500to1m":   { compliance: 1600, monthly: 135 },
    "1mto2m":    { compliance: 1800, monthly: 155 },
    "2mto5m":    { compliance: 2200, monthly: 180 },
  },
  annualDiscount: 0.20,
  transitionFee: 600,
};

export const TA_REVENUE_TIERS: { id: TARevenueKey; label: string }[] = [
...
export function calculateTAPrice(opts: {
  revenueTier: TARevenueKey;
  billing: "monthly" | "annual";
  startDateId: string;
  config?: TAConfig;
}) {
  const cfg = opts.config ?? TA_PRICING;
  const tierCfg = cfg.tiers[opts.revenueTier];
  const startInfo = TA_START_DATES.find((d) => d.id === opts.startDateId) ?? TA_START_DATES[0];
